...
read website;
if grep -q -s -w $website $Website_file;
then
...
then
    ...
    sleep 1
    return
elif [ $website -gt 0 ] && [ $website -le $line_count ]
then
...
then
    ...
    sleep 1
    return
elif [ $websiteOld -gt 0 ] && [ $websiteOld -le $line_count ]
then
    echo -e "${YELLOW}Enter new website name(\"q\" to abort):${END}"
    read websiteNew
    if [[ "$websiteNew" == "q" ]]
    then
        echo "${FAINT}${RED}Aborting...${END}"
        sleep 1
        return
    elif grep -q -s -w $websiteNew $Website_file
    then
...
case $choice in
	...
	3) edit_website;;
	4) check_status;;
	5) echo -e "${BLUE}Thank you for using ${MAGENTA}Monter${END}";
